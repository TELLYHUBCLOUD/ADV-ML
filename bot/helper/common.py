...
class TaskConfig:
    def __init__(self):
        ...
        self.mergextool = self.user_dict.get("MERGE", False)
        self.keep_original = self.user_dict.get("ORIGINAL_FILES", False)
        self.pm_msg = None
        self.file_details = {}
        self.mode = tuple()
    ...
    async def apply_prefix_suffix(self, dl_path):
        """Apply leech prefix and suffix to all files in the path"""
        from re import sub as re_sub
        
        # Get prefix and suffix from user settings
        lprefix = self.user_dict.get("LEECH_PREFIX") or getattr(Config, "LEECH_PREFIX", "")
        lsuffix = self.user_dict.get("LEECH_SUFFIX") or getattr(Config, "LEECH_SUFFIX", "")
        
        # Strip HTML tags and replace \s with space
        if lprefix:
            lprefix = re_sub(r"<.*?>", "", lprefix).replace(r"\s", " ")
        if lsuffix:
            lsuffix = re_sub(r"<.*?>", "", lsuffix).replace(r"\s", " ")
        
        # If no prefix/suffix, return unchanged
        if not lprefix and not lsuffix:
            return dl_path
        ...
        def apply_transform(name):
            ...
            # Apply prefix
            if lprefix and not result.startswith(lprefix):
                result = f"{lprefix}{result}"
            
            # Apply suffix
            if lsuffix:
                name_part, ext = ospath.splitext(result)
                result = f"{name_part}{lsuffix}{ext}"
            
            return result
        
        if self.is_file:
            up_dir, name = dl_path.rsplit("/", 1)
            new_name = apply_transform(name)
            if new_name != name:
                new_path = ospath.join(up_dir, new_name)
                await move(dl_path, new_path)
                return new_path
            ...
            for dirpath, _, files in await sync_to_async(walk, dl_path, topdown=False):
                for file_ in files:
                    f_path = ospath.join(dirpath, file_)
                    new_name = apply_transform(file_)
                    if new_name != file_:
                        await move(f_path, ospath.join(dirpath, new_name))
            return dl_path
