...
tmv_direct = """<b>TMV Direct Download</b>:

Download files from TMV (TamilMV) links using direct download method.
The bot will automatically scrape the download link and leech to Telegram.
...
• All standard leech flags supported (-e, -z, -m, -n, -up, -audr, etc.)

<b>Examples:</b>
/tdl https://www.cyberloom.best/l/xyz
/tdl https://www.cyberloom.best/l/xyz -e -m Tamil Movies
/tdl <link> -n Custom.Name.mkv -audr
/tdl <link> -z password -sp 2gb

<b>Note:</b>
• Always leeches to Telegram (not mirror)
• Supports all standard leech flags and options
• Automatically extracts clean filename from page
• Link must be valid TMV/cyberloom link"""
...
audio_remove = """<b>Audio Remove</b>: -audr
Remove specific audio tracks from MKV files using interactive mode.

<b>Usage:</b>
/cmd link -audr

<b>How it works:</b>
• Download completes first
• Shows interactive button menu with all audio tracks
• Select tracks to REMOVE (multiple selection)
• Optionally rename the file
• Click Done to proceed

<b>Features:</b>
• Visual selection with language codes and titles
• Multiple track selection
• Built-in rename option
• 3-minute timeout

Note: Only works with MKV files. Track indices start from 0."""

audio_change = """<b>Audio Order Change</b>: -audc
Change the order of audio tracks in MKV files.
/cmd link -audc 1,0 (swap first and second audio tracks)
/cmd link -audc 2,0,1 (reorder three audio tracks)
Note: Only works with MKV files. Specify the new order using comma-separated indices."""

merge_name = """<b>Merge Name</b>: -nm
Set a custom name for merged video files when using MERGE setting.
/cmd link -nm MyMovie (merged file will be named MyMovie.mkv)
/cmd link -nm Custom Name (merged file will be named Custom Name.mkv)
Note: Only applies when MERGE is enabled in user settings and multiple videos are merged."""
...
def get_bot_commands():
    ...
    static_commands = {
        ...
        "TmvDirect": "[link] Leech TMV links directly to Telegram",
...
def get_help_string():
    ...
    for key, cmds in commands.items():
        ...
        if key == "SpeedTest" and key in BOT_COMMANDS:
            ...
            help_lines.append(f"{cmd_str}: Leech TMV direct download links.")
        elif key == "Clone":
            help_lines.append(
