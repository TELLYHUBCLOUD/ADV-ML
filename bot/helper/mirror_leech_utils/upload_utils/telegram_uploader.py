...
from ...ext_utils.user_shortener import user_short_url
...
class TelegramUploader:
    def __init__(self, listener, path, gofile_links=None, devuploads_links=None):
        self._last_uploaded = 0
        self._processed_bytes = 0
        self._listener = listener
        self._path = path
        self._gofile_links = gofile_links or {}  # Pre-uploaded gofile links
        self._devuploads_links = devuploads_links or {}  # Pre-uploaded devuploads links
        self._client = None
...
╰ <b>Source :</b> <a href='{self._listener.source_url}'>Click Here</a>"""
    ...
    async def _prepare_file(self, pre_file_, dirpath, gofile_link=None, devuploads_link=None):
        # Use filename as-is from disk (already has all transformations applied)
        file_ = pre_file_
        
        # Shorten links if enabled by user
        user_dict = self._listener.user_dict
        if gofile_link:
            gofile_link = await user_short_url(gofile_link, user_dict)
        if devuploads_link:
            devuploads_link = await user_short_url(devuploads_link, user_dict)

        cap_mono = (
            f"<{Config.LEECH_FONT}>{file_}</{Config.LEECH_FONT}>"
        ...
        if self._lcaption:
            ...
            cap_mono = parts[0].format(
                ...
                gofile_link=gofile_link if gofile_link else "N/A",
                devuploads_link=devuploads_link if devuploads_link else "N/A",
            )
        ...
        # If gofile link exists but wasn't used in custom caption, append it
        if gofile_link and self._lcaption and "{gofile_link}" not in self._lcaption.lower():
            cap_mono += f"\n\nGofile Link : {gofile_link}"
        elif gofile_link and not self._lcaption:
            # If no custom caption but gofile link exists, add it to filename
            cap_mono += f"\n\nGofile Link : {gofile_link}"
        
        # If devuploads link exists but wasn't used in custom caption, append it
        if devuploads_link and self._lcaption and "{devuploads_link}" not in self._lcaption.lower():
            cap_mono += f"\n\nDevUploads Link : {devuploads_link}"
        elif devuploads_link and not self._lcaption:
            # If no custom caption but devuploads link exists, add it to filename
            cap_mono += f"\n\nDevUploads Link : {devuploads_link}"

        # Handle Telegram 56-char filename limit (only for telegram, not external uploads)
        if len(file_) > 56:
            if is_archive(file_):
            ...
            # Rename file on disk if needed for telegram
            if pre_file_ != file_:
    ...
    async def _send_media_group(self, subkey, key, msgs):
        ...
        # Use self._sent_msg to reply to, handle cases where reply_to_message might be None
        reply_msg = msgs[0].reply_to_message if msgs[0].reply_to_message else self._sent_msg
        msgs_list = await reply_msg.reply_media_group(
    ...
    async def upload(self):
        ...
        for dirpath, _, files in natsorted(await sync_to_async(walk, self._path)):
            ...
            for file_ in natsorted(files):
                # Skip gofile temporary files
                if file_.startswith("gofile_temp_"):
                    LOGGER.info(f"Skipping gofile temp file: {file_}")
                    continue
                ...
                try:
                    ...
                    # Use filename directly from disk (already has all transformations applied)
                    # Get pre-uploaded gofile link for this file
                    gofile_url = self._gofile_links.get(file_, None)
                    
                    # Get pre-uploaded devuploads link for this file
                    devuploads_url = self._devuploads_links.get(file_, None)
                    
                    cap_mono = await self._prepare_file(file_, dirpath, gofile_url, devuploads_url)
