...
class GoFileUpload:
    def __init__(self, listener, path, skip_token_validation=False):
        ...
        self.skip_token_validation = skip_token_validation  # Skip validation if already validated
    ...
    async def upload_file(
        ...
        # Replace spaces with dots in filename for upload
        original_path = path
        ...
        # Rename back to original name after upload
        await aiorename(new_path, original_path)
        
        return await self.__resp_handler(upload_file)

    async def _upload_dir(self, input_directory, parent_folder_id=None, root_folder_id=None):
    ...
    async def _upload_process(self):
        # Skip token validation if already validated (e.g., in batch upload)
        account_data = None
        ...
        if await aiopath.isfile(self._path):
            # Single file upload
            folder_id = self.folder_id or (account_data["rootFolder"] if account_data else None)
            ...
            folder_code = await self._upload_dir(
                self._path, root_folder_id=(account_data["rootFolder"] if account_data else None)
            )
            if folder_code:
