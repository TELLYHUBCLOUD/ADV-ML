...
from ..ext_utils.attachment_utils import apply_attachment
from ..ext_utils.audio_utils import (
    apply_audio_removal,
    apply_audio_order_change,
    apply_audio_removal_interactive,
)
...
from ..mirror_leech_utils.status_utils.gofile_batch_status import GofileBatchStatus
from ..mirror_leech_utils.status_utils.devuploads_batch_status import (
    DevUploadsBatchStatus,
)
...
from ..mirror_leech_utils.upload_utils.gofile_batch_uploader import GofileBatchUploader
from ..mirror_leech_utils.upload_utils.devuploads_batch_uploader import (
    DevUploadsBatchUploader,
)
...
from ..ext_utils.merge_videos import Merge
...
""",
 ...
 """,
    ...
    async def on_download_complete(self):
        ...
        # Apply audio removal from -audr flag (always interactive mode)
        if hasattr(self, "audio_remove") and self.audio_remove:
            up_path = await apply_audio_removal_interactive(
                self,
                up_path,
                gid,
            )
        ...
        # Apply audio order change from -audc flag
        if hasattr(self, "audio_change") and self.audio_change:
            up_path = await apply_audio_order_change(
                self,
                up_path,
                gid,
                self.audio_change,
        ...
        if self.mergextool and not self.compress:
            ...
            # Smart merge check for leech only
            if self.is_leech and self.user_dict.get("SMART_MERGE", False):
                from ..ext_utils.media_utils import get_document_type
                from os import walk

                # Calculate total size of video files that would be merged
                total_video_size = 0
                video_count = 0
                for dirpath, _, files in await sync_to_async(
                    walk, up_path if not self.is_file else ospath.dirname(up_path)
                ):
                    for file in files:
                        video_file = ospath.join(dirpath, file)
                        doc_type = await get_document_type(video_file)
                        if doc_type[0]:  # Is video file
                            total_video_size += await get_path_size(video_file)
                            video_count += 1

                # Skip merge if total size > 4GB and there are multiple videos
                if video_count > 1 and total_video_size > 4 * 1024 * 1024 * 1024:
                    should_merge = False
                    LOGGER.info(
                        f"Smart Merge: Skipping merge for {self.name} - Total video size {get_readable_file_size(total_video_size)} > 4GB. Will upload {video_count} separate files."
                    )

            if should_merge:
                up_path = await Merge(self).merge_vids(
                    up_path,
                    gid,
                    keep_original=self.keep_original,
                    custom_name=getattr(self, "new_name", None),
                )
                if up_path is False:
                    await self.on_upload_error("Failed to merge videos!")
                    return
                self.is_file = await aiopath.isfile(up_path)
                if self.is_file:
        ...
        # Apply name_swap (after merge, before prefix/suffix)
        if self.name_swap:
        ...
        # Apply prefix/suffix (after nameswap, before metadata) - for leech only
        if self.is_leech:
            up_path = await self.apply_prefix_suffix(up_path)
            if self.is_cancelled:
                return
            self.is_file = await aiopath.isfile(up_path)
            self.name = up_path.replace(f"{up_dir}/", "").split("/", 1)[0]

        # Apply metadata (after nameswap and prefix/suffix)
        if (
        ...
        if self.is_yt:
            ...
            # Check for Gofile upload
            if self.user_dict.get("GOFILE_LEECH_ENABLED") and self.user_dict.get(
                "GOFILE_TOKEN"
            ):
                LOGGER.info("Starting batch gofile upload...")

                # Create GofileBatchUploader and set status
                gofile_uploader = GofileBatchUploader(self)
                async with task_dict_lock:
                    task_dict[self.mid] = GofileBatchStatus(self, gofile_uploader, gid)
                await update_status_message(self.message.chat.id)

                # Upload all files to gofile
                gofile_links = await gofile_uploader.upload_all_files(up_dir)
                LOGGER.info(
                    f"Gofile batch upload completed. Uploaded {len(gofile_links)} files."
                )

            # Check for DevUploads upload
            if self.user_dict.get("DEVUPLOADS_LEECH_ENABLED") and self.user_dict.get(
                "DEVUPLOADS_API_KEY"
            ):
                LOGGER.info("Starting batch DevUploads upload...")

                # Create DevUploadsBatchUploader and set status
                devuploads_uploader = DevUploadsBatchUploader(self)
                async with task_dict_lock:
                    task_dict[self.mid] = DevUploadsBatchStatus(
                        self, devuploads_uploader, gid
                    )
                await update_status_message(self.message.chat.id)

                # Upload all files to devuploads
                devuploads_links = await devuploads_uploader.upload_all_files(up_dir)
                LOGGER.info(
                    f"DevUploads batch upload completed. Uploaded {len(devuploads_links)} files."
                )

            # Create TelegramUploader with gofile or devuploads links
            tg = TelegramUploader(self, up_dir, gofile_links, devuploads_links)
            async with task_dict_lock:
