...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        # Audio remove flag - always use interactive mode when flag is present
        # Check if -audr is in the original input (not default empty string)
        audr_value = args["-audr"]
        # Flag is present if it's not the default empty string or is True
        if audr_value != "" or "-audr" in " ".join(input_list):
            self.audio_remove = "interactive"
            LOGGER.info(f"Audio remove interactive mode enabled. Flag value: {audr_value}")
        else:
