        ...
        for status, speed, _ in status_results:
            match status:
                ...
                case MirrorStatus.STATUS_FFMPEG:
                    tasks["FFmpeg"] += 1
                case MirrorStatus.STATUS_MERGING:
                    tasks["Merge"] += 1
                case MirrorStatus.STATUS_ATTACHMENT:
                    tasks["Attachment"] += 1
                case MirrorStatus.STATUS_AUDIO_REMOVE:
                    tasks["AudioRemove"] += 1
                case MirrorStatus.STATUS_AUDIO_CHANGE:
                    tasks["AudioChange"] += 1
                case "GofileUp":
                    tasks["GofileUp"] += 1
                    up_speed += speed_string_to_bytes(speed)
...
┠ <b>Merge:</b> {tasks["Merge"]} | <b>Attachment:</b> {tasks["Attachment"]}
┠ <b>AudioRemove:</b> {tasks["AudioRemove"]} | <b>AudioChange:</b> {tasks["AudioChange"]}
┠ <b>GofileUp:</b> {tasks["GofileUp"]}
┞
│
