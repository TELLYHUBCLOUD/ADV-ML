...
devuploads_options = ["DEVUPLOADS_API_KEY", "DEVUPLOADS_LEECH_ENABLED"]
shortener_options = ["SHORTENER_URL", "SHORTENER_API", "SHORTENER_ENABLED"]
uphoster_options = [
    "GOFILE_TOKEN",
    "GOFILE_FOLDER_ID",
...
⏱ <b>Time Left:</b> <code>60 sec</code>""",
    ...
    "ATTACHMENT_URL": (
        "📎 Attachment URL",
        "Attach an image (cover/poster) to all MKV files from URL.",
        """<i>📷 Send the direct URL of the image to attach to MKV files</i>

<b>💡 Example URLs:</b>
• <code>https://i.ibb.co/abc123/image.png</code>
• <code>https://example.com/poster.jpg</code>

<b>ℹ️ Supported formats:</b> JPG, JPEG, PNG

<b>📝 Note:</b> This image will be embedded as an attachment in the MKV container (like a cover art)
...
⏱ <b>Time Left:</b> <code>60 sec</code>""",
    ...
    "GOFILE_LEECH_ENABLED": (
        "Bool",
        "Auto Upload to Gofile on Leech",
        "<i>Enable auto upload to Gofile when using leech commands.</i>",
    ),
    "DEVUPLOADS_API_KEY": (
        "String",
        "DevUploads API Key",
    ...
    "DEVUPLOADS_LEECH_ENABLED": (
        "Bool",
        "Auto Upload to DevUploads on Leech",
        "<i>Enable auto upload to DevUploads when using leech commands.</i>",
    ),
    "SHORTENER_URL": (
        "String",
        "Shortener URL",
    ...
    "SHORTENER_ENABLED": (
        "Bool",
        "Enable Link Shortener",
        "<i>Enable shortening of Gofile and DevUploads links.</i>",
    ),
    "BUZZHEAVIER_TOKEN": (
        "String",
    ...
    "SMART_MERGE": (
        "Bool",
        "Smart Merge for Leech",
        "<i>Automatically skip merge when total video size exceeds 4GB for leech operations. This prevents merge+split cycles and uploads separate files directly.</i>",
    ),
}
...
"""
        ...
        # Get gofile and devuploads leech settings
        gofile_leech_enabled = user_dict.get("GOFILE_LEECH_ENABLED", False)
        devuploads_leech_enabled = user_dict.get("DEVUPLOADS_LEECH_ENABLED", False)

        # Toggle button for gofile leech
        toggle_text = "✅ Gofile" if gofile_leech_enabled else "❌ Gofile"
        buttons.data_button(
            toggle_text,
            f"userset {user_id} tog GOFILE_LEECH_ENABLED {'f' if gofile_leech_enabled else 't'}",
        )

        # DevUploads Settings button
        buttons.data_button("DevUploads Settings", f"userset {user_id} devuploads")

        # Shortener Settings button (common for Gofile and DevUploads)
        buttons.data_button("Shortener Settings", f"userset {user_id} shortener")

        # Smart Merge Settings button
        buttons.data_button("Smart Merge", f"userset {user_id} smartmerge")
...
╰ <b>Gofile Folder ID</b> » <code>{gffolder}</code>"""
        ...
        buttons.data_button(
            "DevUploads API Key", f"userset {user_id} menu DEVUPLOADS_API_KEY"
        )

        # Toggle button for devuploads leech
        devuploads_leech_enabled = user_dict.get("DEVUPLOADS_LEECH_ENABLED", False)
        toggle_text_dev = "✅ Enabled" if devuploads_leech_enabled else "❌ Disabled"
        buttons.data_button(
            toggle_text_dev,
            f"userset {user_id} tog DEVUPLOADS_LEECH_ENABLED {'f' if devuploads_leech_enabled else 't'}",
        )
        ...
        if user_dict.get("DEVUPLOADS_API_KEY", False):
            devtoken = user_dict["DEVUPLOADS_API_KEY"]
        elif Config.DEVUPLOADS_API_KEY:
            devtoken = Config.DEVUPLOADS_API_KEY
        else:
            devtoken = "None"
...
╰ <b>DevUploads API Key</b> » <code>{devtoken}</code>"""
        ...
        buttons.data_button(
            "Shortener API Key", f"userset {user_id} menu SHORTENER_API"
        )

        shortener_url = user_dict.get("SHORTENER_URL", "")
        shortener_api = user_dict.get("SHORTENER_API", "")

        # Only show enable toggle if both URL and API are set
        if shortener_url and shortener_api:
            shortener_enabled = user_dict.get("SHORTENER_ENABLED", False)
            toggle_text_shortener = "✅ Enabled" if shortener_enabled else "❌ Disabled"
            buttons.data_button(
                toggle_text_shortener,
                f"userset {user_id} tog SHORTENER_ENABLED {'f' if shortener_enabled else 't'}",
            )
        else:
            shortener_enabled = False
            toggle_text_shortener = "❌ Set URL & API first"

        buttons.data_button("Back", f"userset {user_id} back leech", "footer")
        buttons.data_button("Close", f"userset {user_id} close", "footer")
        btns = buttons.build_menu(1)

        shortener_url_display = shortener_url or "None"
...
╰ <b>Status</b> » {toggle_text_shortener}"""
        ...
        toggle_text_sm = "✅ Enabled" if smart_merge_enabled else "❌ Disabled"
        
        buttons.data_button(
            toggle_text_sm,
            f"userset {user_id} tog SMART_MERGE {'f' if smart_merge_enabled else 't'}",
        )
...
"""
        ...
        buttons.data_button("Attachment URL", f"userset {user_id} menu ATTACHMENT_URL")
        ...
        attachment_url = user_dict.get("ATTACHMENT_URL")
        display_attachment = "<b>Not Set</b>"
        if attachment_url:
            # Truncate long URLs for display
            if len(attachment_url) > 50:
                display_attachment = f"<code>{escape(attachment_url[:47])}...</code>"
            else:
                display_attachment = f"<code>{escape(attachment_url)}</code>"

        # Merge Settings - Toggle Buttons
        if user_dict.get("MERGE", False):
            buttons.data_button("✅ Disable Merge", f"userset {user_id} tog MERGE f")
            merge_enabled = "Enabled"
            # Show Keep Original button only when merge is enabled
            if user_dict.get("ORIGINAL_FILES", False):
                buttons.data_button(
                    "✅ Don't Keep Original", f"userset {user_id} tog ORIGINAL_FILES f"
                )
                keep_orig = "Enabled"
            else:
                buttons.data_button(
                    "❌ Keep Original Files", f"userset {user_id} tog ORIGINAL_FILES t"
                )
                keep_orig = "Disabled"
        else:
            buttons.data_button("❌ Enable Merge", f"userset {user_id} tog MERGE t")
            merge_enabled = "Disabled"
            keep_orig = "Disabled"
    ...
    # elif option == "LEECH_DUMP_CHAT": # TODO: Add
        ...
        if value and isinstance(value, str):
            ...
            if not any(
                ...
                # Allow URLs without extensions (like image hosting services with query params)
                if "?" not in value and not any(
                    domain in value.lower() for domain in ["ibb.co", "imgur", "postimg"]
                ):
                    await send_message(
                        message,
                        "Warning: URL doesn't end with a common image extension (.jpg, .jpeg, .png). Make sure it's a direct image link.",
                    )
        else:
            value = ""
...
async def get_menu(option, message, user_id):
    ...
    if option in devuploads_options:
        back_to = "devuploads"
    elif option in shortener_options:
        back_to = "shortener"
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        if data[3] == "STOP_DUPLICATE":
            ...
            back_to = "general"
        elif data[3] in ["MERGE", "ORIGINAL_FILES"]:
            back_to = "ffset"
        elif data[3] == "GOFILE_LEECH_ENABLED":
            back_to = "leech"
        elif data[3] == "DEVUPLOADS_LEECH_ENABLED":
            back_to = "devuploads"
        elif data[3] == "SHORTENER_ENABLED":
            back_to = "shortener"
        elif data[3] == "SMART_MERGE":
            back_to = "smartmerge"
        else:
            back_to = "leech"
