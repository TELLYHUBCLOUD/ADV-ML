...
def add_handlers():
    ...
    from ..helper.ext_utils.audio_utils import setup_audio_selection_handler

    setup_audio_selection_handler()
    TgClient.bot.add_handler(
        MessageHandler(
            remove_from_queue,
    ...
    TgClient.bot.add_handler(
        MessageHandler(
            tmv_direct_leech,
            filters=command(BotCommands.TmvDirectCommand, case_sensitive=True)
            & CustomFilters.authorized,
        )
    )
    TgClient.bot.add_handler(
